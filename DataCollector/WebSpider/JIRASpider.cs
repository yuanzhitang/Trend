...
{
	public class JIRASpider : IWebSpider
	{
		...
		public JIRASpider(string productName, string sprintHomeURL)
		{
		...
		private Dictionary<string, string> GetSprintItems()
		{
			...
			var menu = htmlNode.SelectSingleNode("//*[@id='ghx-content-group']/div[1]");
		...
		private void LoadStoryTaskInfomation(Story story)
		{
			...
			foreach (var taskItem in taskList)
			{
				...
				task.Owner = owner;
				task.Status = (TaskStatus)Enum.Parse(typeof(Unisys.Trend.DataModel.TaskStatus), status.Replace(" ", ""));
		...
		{
			string cookieData = CookieUtil.GetCookies(ConfigManager.GetRootUrl());
